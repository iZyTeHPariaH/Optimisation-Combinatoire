...
branchbound branch bound borne initN opt@(optimum,optval) criteria
     -- Si on ne peut pas espérer trouver mieux dans l'arbre, on abandonne la recherche.
     |  optval `better` maxval = return (optimum,optval)
     ...
     | maxval `better` minval = trace ("Branching on " ++ show initN)
                                      (foldM (\a e -> branchbound branch bound borne e a criteria) opt (branch initN))
                          
     -- Sinon, si on est sur que le noeud contient une solution optimale, on ne l'explore pas et on
     -- le retourne
     | otherwise        = return (initN,minval)
  where minval = borne initN
        maxval = bound initN
        initVal = solve initN
